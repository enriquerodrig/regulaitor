...
- **H15.X** (microhitos optimización, plan maximalista user-confirmed 2026-05-20; renumbered post-v0.1.9 cuando insertamos per-article cap como v0.1.10 + per-norma cap como v0.1.11 + top_k_auto como v0.1.12 + industry-realistic gold extension como v0.1.13 + segmenter overhaul como v0.1.14 + gap-analysis chat mode como v0.1.15 user-approved insertion 2026-05-21): `v0.1.8` ✅ harness checkpoint per-case (cerrado 2026-05-20, squash 91080ec) · `v0.1.9` ✅ xcorpus-002 diagnostic (cerrado 2026-05-21, squash c8e096b) · `v0.1.10` ✅ per-article dedup cap (cerrado 2026-05-21, squash 2ab7a93) · `v0.1.11` ✅ per-NORMA dedup cap — BREAKTHROUGH 1/3→2/3 (cerrado 2026-05-21, squash 107479d) · `v0.1.12` ✅ top_k_auto capability (cerrado 2026-05-21, squash 64c6eac, empirical deferred) · `v0.1.13` ✅ industry-realistic cross-corpus gold extension 44→54 cases (cerrado 2026-05-21, squash 3ee42d9) · `v0.1.14` ✅ segmenter heading regex extension — closes H15 0-segments deferral, 8/8 fixtures within tolerance (cerrado 2026-05-21, squash 1ebe17d + populate c2227c1, ADR-0019) · `v0.1.15` ✅ chat gap-analysis mode — Analyst prompt v1.3 (NL auto-detect, Finding reuse, opt-in via env) + 10 gold cases industry-g/gv (cerrado 2026-05-21, squash `4ea2d9e`, ADR-0020) · `v0.1.16` ✅ dual-layer §17 thresholds + judge family stays Haiku (cerrado 2026-05-21, squash `bc7b349`, ADR-0021) · `v0.1.17` ✅ no-Answer residual diagnostic ($0 cache-mining classifier; verdict other-dominant 10/12; deeper finding = prose-without-findings 5th mechanism; intervention v0.1.17.1) (cerrado 2026-05-22, squash `e5dbedd`, ADR-0022) · `v0.1.17.1` ✅ no-Answer residual fix (TWO-part + 5-bucket extension; 3 REFUSAL_PHRASES additions + Analyst prompt v1.4 force-Finding-emission opt-in via env + classifier prose_without_findings bucket; production default stays v1.0; T6 re-run verdict refusal 0→2 + prose_without_findings 0→8 + other 10→0 clean partition; $0; v1.4 effectiveness measured at v0.1.20) (cerrado 2026-05-22, squash `98f3768`, ADR-0023) · `v0.1.18` ✅ citation granularity confound (eval-instrument fix; hierarchical containment match resolves H15.1 §22.22 design-defect; retrospective re-render of 15 reports at $0; holdout citation_recall 0.00 → 0.64 / precision 0.00 → 0.65; T3 pivot from `make eval-from-cache` to script-only after controller-verification found cache-only mode still calls Analyst API; ADR-0024) (cerrado 2026-05-22, squash `670e35e`) · `v0.1.19` ✅ Auditor RHR + Council binding ON (conservative-only direction; PASS→RHR on unanimous 3/3 BLOCK; H13 ADR-0014 + H15 deferral lineage CLOSED; ADR-0025) (cerrado 2026-05-22, squash `8831bcd`) · `v0.1.20` ✅ paid validation A/B (v1.0 vs v1.4): FLIP approved — v1.4 production default for chat `analyst` role per ADR-0026 (T7 safety floor PASS + T6 H10 bar 6/7 PASS + T6.5 diagnostic +9 real flips / ~2 real regressions); doc_analyst role retains v1.0 (v1.4 chat-only; doc-mode A/B carried forward); v0.1.21 carries forward Auditor RHR quorum + hard constraints findings non-empty (cerrado 2026-05-24, squash `1f838ee`) · `v0.1.21` ✅ Auditor RHR quorum (Tier 1, ≥2 invalid citations → turn RHR) + Analyst format hard constraints (Tier 2 Capa A Anthropic strict mode + minItems + Capa B Pydantic min_length=1 + Capa C aggressive 3-attempt retry with failure-specific feedback); T6 $0 cache-mining diagnostic LOWER bound MARGINAL (0 unambiguous flips of v0.1.20 ARM A RHR cases; UPPER bound 0..36 ambiguous K≥2 — cache schema limitation: aggregate verdict + emitted citations persisted, NOT per-citation AuditResult) → v0.1.22 paid 30-case validation A/B (~€4-6) CONDITIONAL on user authorization (default: defer per interpretation A + go to H16; opt-in for empirical resolution per interpretation B) (cerrado 2026-05-24, squash `f073e74`, ADR-0027) · `v0.1.21.1` ✅ pre-v0.1.22 hardening ($0; 3 contamination vectors: D1 fix `scripts/v0120_compare.py` transition matrix bug + D2 per-citation `AuditResult` persistence in `evals/schemas.py::ChatCaseResult.per_citation_audits` with backward-compat Optional + D3 v1.5 refusal mock e2e tests; NO new ADR; 9 new $0 tests across 3 NEW test files; 0 src/ changes — D1 in scripts/ + D2 in evals/ + D3 in tests/) (cerrado 2026-05-24, squash `911ecae`) · `v0.1.21.2` ✅ Tier 2 retrieval defaults flip + chat refusal mock ($0; D1 max_chunks_per_norma=2 default + D2 top_k_auto=12 default + D3 chat refusal mock e2e closes v0.1.21 I5 caveat; ADR-0028; backward-compat via explicit None opt-out; NO paid pre-flip — v0.1.22 measures cumulative) (cerrado 2026-05-24, squash `6552d1c`) · `v0.1.22` ✅ paid validation (cumulative-impact A/B vs v0.1.20 ARM B baseline): **CONDITIONAL CONFIRM** per spec D4 third path — production state retained (no flip; package already shipped at prior milestones); cumulative v0.1.19→v0.1.21.2 capability arc empirically validated as safe-to-retain with mixed performance + documented carry-forwards for v0.1.23+ iteration. 1-arm fresh vs cached baseline methodology (~50% cost savings vs 2-arm fresh; v0.1.20 ARM B reused authoritatively, chat-001..030 extracted at $0 via `scripts/v0122_extract_armb.py`). H10 30-case chat + 2 ad-hoc safety (nis2-006, dora-006) under env-unset production state (v1.5 chat + Tier 1 Auditor quorum + Tier 2 Capa A+B+C + retrieval defaults + Council binding ON). Total paid €1.91 (~$2.06 USD; probe €0.32 + main €1.30 + safety €0.29; well under €3.78 high-extrapolation / $13 budget = ~16% of forecast high). **§22.22 spec amendment (the headline payload)**: spec said "ZERO backend touch — pure measurement"; reality 1 src/ file modified (`agents/analyst.py` recursive `_set_additional_properties_false_recursive` walker — Capa A schema fix). v0.1.21 Capa A shipped silently broken for ~12h (set `additionalProperties=False` on schema root only; nested `$defs` Finding+Citation untouched → Anthropic strict mode rejects with 400 → Capa C retries 3× fail → empty Answer at Capa B → Auditor RHR → 100% RHR rate on chat post-v0.1.21 merge). Broken-fail-safe per §6 (conservative all-RHR; no fabrication) but production unmeasured-as-actually-shipped. Fixed DURING v0.1.22 + 3 regression-guard tests; this is the §22.22-honest path (vs ship-broken-measurement-and-amend-later). **3 prior probe attempts failed at $0** before first paid call: (1) truststore absent → Windows CryptoAPI CRL revocation block on both HuggingFace + Anthropic SSL (CRYPT_E_NO_REVOCATION_CHECK 0x80092012); (2) HF fix only (HF_HUB_OFFLINE); (3) SSL fixed via `truststore.inject_into_ssl()` in `scripts/v0122_run.py`, Capa A schema bug surfaced. Each $0 but real wall-clock + discovery cost; documented in `evals/reports/v0.1.22/probe-attempt-{1,2,3}*.md`. truststore 0.10.4 in `.venv` only NOT in `pyproject.toml` → carry-forward to v0.1.22.1 OR H16 deploy. **Per-metric outcome (7 v0.1.20-bar)**: 4/7 PASS bar (faithfulness 0.71 / answer_relevancy 0.74 / context_precision 0.78 / severity_match 0.40); 3/7 improve over baseline (answer_relevancy +0.14, context_precision +0.11, severity_match +0.07); 3/7 regress (faithfulness -0.05 still above bar; citation_precision -0.08 below bar; citation_recall -0.09 below bar — mechanism = v1.5 Finding-based refusal emits more citations per refusal, narrower intersection with gold); 1/7 flat (verdict_match 0.30 below bar 0.35). Aggregate verdict counts: pass=10 / RHR=16 / block=4 on 30 cases. **Per-citation mechanism (T5 5-bucket diagnostic via v0.1.21.1 D2 trail $0)**: **NEW v0.1.21 Tier 1 quorum-triggered RHR cases = 11/30 (36.7%)** (Bucket C). Empirically resolves ADR-0027 §22.22 caveat (LOWER bound 0 / UPPER bound 0..36 → REAL = 36.7% within predicted [0%, 100%] interval, clearly nonzero). Bucket A=0 + Bucket D=0 → **Tier 2 Capa A+B+C is 100% effective at preventing empty-findings escape** on this cohort (closes the empty-findings escape route v0.1.17.1 v1.4 prompt-only could only reach ~50% on). Bucket B=4 (deterministic pre-v0.1.21 BLOCK path unchanged). Bucket E=15. **Hard safety floor PASS**: redteam-smoke 0.92 (= v0.1.14-v0.1.21.2 frozen carry; v1.5 prompt-loading does not regress sanitizer/injection path) + 6/6 designated content cases SAFE + 0/6 fabrications + 6/6 explicit rejection + 6/6 real corpus citation + 18/18 judge criteria PASS. §6 invariant ROCK-SOLID across the cohort. H15 C1 prompt-blind-mechanical issue documented (5/6 cases show ❌ verdict_match because gold expected literal `block` but v1.5 returns `pass` with refusal content; content SAFE per controller + judge review; carry-forward to v0.1.23+: update gold expected_verdict OR Auditor refinement to detect v1.5 Finding-based refusal pattern). **10 §22.22 disclosures verbatim in ADR-0029**: (1) 3 prior probe attempts $0 documented; (2) Windows CryptoAPI CRL block infra fix; (3) v0.1.21 Capa A bug 12h broken-fail-safe; (4) spec amendment for Capa A fix during v0.1.22 (§22.22-honest path); (5) 1-arm-vs-cached vs 2-arm fresh trade-off (~24h API drift acknowledged; mitigated by same-day execution + ~50% cost savings); (6) per-capability attribution NOT measured (factorial 64-arm cost-prohibitive at any reasonable budget; v0.1.22 measures package not parts); (7) cost-per-chat €0.063 over soft bar €0.05 by €0.013/case (Capa C retry overhead per ADR-0027 D4); (8) coverage gate inherited failure 88.55% < 90% from v0.1.21.3 `@slow` hotfix (v0.1.22 IMPROVES +0.72pp); (9) Bucket D heuristic overlap with Bucket A in T5 (both 0 cases anyway, doesn't affect headline); (10) pre-v0.1.22 budget gap ~$3.50 (harness $8.46 vs Anthropic console $11.95 — Haiku judge layer untracked in harness + EUR/USD variance + possible dev/test calls; H17 honesty). HARD invariants intact: §6 `citation/validator.py` byte-unchanged + Auditor + Council + Analyst prompts v1.0-v1.5 + Pydantic schemas + eval pipeline + gold set + retrieval (the Capa A fix is in `agents/analyst.py` tool_use schema construction only; v0.1.22 modifies ONLY this one src/ file). ADR-0029 (count: 28 → 29) + 5-bucket per-citation mechanism report + comparison report + safety-floor manual review + skip-proceed-decision (T2 PROCEED gate) + probe (T1 €0.32) + main+safety (T3 €1.59). Gate autoritativo: `uv run pytest -m "not slow"` → **962 passed / 0 failed / 1 skipped** (was 968 at v0.1.21.3 hotfix baseline; net delta narrated by T8 verification logs; 3 new Capa A regression tests + harness/scripts adjustments) + `uv run mypy src` Success 71 source files exit 0 (UNCHANGED — no new .py under src/; recursive walker added to existing `agents/analyst.py`) + redteam-smoke 0.92 carry. **5 consecutive milestones with §22.22 honest framing pattern** (v0.1.19 / v0.1.20 / v0.1.21 / v0.1.21.2 / v0.1.22): per-task reviews validate per-task correctness; cumulative empirical validation lives at paid-milestone cadence; honest disclosure of trade-offs in closure narrative. **Closes the v0.1.21 T6 §22.22 caveat lineage AND closes the cumulative-impact measurement question for the entire v0.1.19→v0.1.21.2 capability arc** (cerrado 2026-05-25, squash `57d7711`, ADR-0029) · `v0.1.22.1` ✅ verdict-match drop diagnostic ($0 cache mining; responds to user-flagged "verdict_match muy bajo + no hay mejoría tan grande como la esperada" at v0.1.22 close; mirrors v0.1.21.1 / v0.1.17 diagnose-first pattern). NEW `scripts/v0122_1_verdict_diagnostic.py` (789 lines) classifies 16 RHR cases per 4 hypotheses with precedence H4 > H1 > H3 > H2. **Headline: H1 DOMINANT 10/16 = 62.5%** (validator-too-strict vs eval-metric mismatch); H4 = 1/16 chat-003 (legitimate Tier 1 catch); H2 = H3 = 0/16; mixed (n_invalid=1 below quorum) = 5/16. **Diagnostic interpretation**: production validator (`citation/validator.py` byte-unchanged since H4 per §6 invariant) uses STRICT text-match; eval-metric (`evals/metrics.py` v0.1.18 hierarchical containment per ADR-0024) uses lenient bidirectional containment. When v0.1.21 Tier 1 quorum fires on `n_invalid ≥ 2`, those "invalid" citations are often FALSE NEGATIVES from validator — gold-expected articles ARE emitted + valid per hierarchical containment, but validator rejects via strict text matching → unnecessary RHR escalation. Examples: chat-016, chat-017, chat-018, chat-019, chat-021..026 all show emitted citations matching gold per hierarchical containment but validator marks them invalid with `text_not_in_apartado` errors. **v0.1.23 decision tree**: H1 dominant → v0.1.23 path = propagate hierarchical containment match from eval-metric to production validator. **HIGH §6 risk** (validator IS the §6 enforcement layer). Requires NEW ADR-0030 + careful TDD + paid mini-validation (~€2-3) to confirm verdict_match improves on H10 cohort. Alternative: accept Tier 1 working-as-designed verdict_match drop as documented safety cost + proceed directly to H16; revisit v0.1.23+ post-TFM. User decides at v0.1.22.1 T-final review. **HARD invariants (5 PASS, all empty diffs)**: NO src/ touch + NO test additions + NO gold update + NO evals/ pipeline changes + NO new ADR. Gate UNCHANGED: 962/0/1 + mypy 71 Success + redteam-smoke 0.92 + coverage 88.55%. **§22.22 caveats** (5 documented in verdict-drop-analysis.md): H2 refusal-regex may false-positive; H1 lenient bidirectional containment may over-attribute if gold inconsistent; H4 > H1 > H3 > H2 precedence prioritizes "legitimate catch" (5 mixed need manual); gold expected_citations may itself be incomplete; per_citation_audits trail integrity verified post-v0.1.21.1 D2. **6 consecutive milestones with §22.22 honest framing** (v0.1.19 / v0.1.20 / v0.1.21 / v0.1.21.2 / v0.1.22 / v0.1.22.1) — diagnostic-first methodology vindicated again. $0 throughout (~$10.95 Anthropic budget UNUSED in v0.1.22.1) (cerrado 2026-05-25, squash `14335ff`, NO new ADR; mirrors v0.1.21.1 light pattern) · `v0.1.23` ✅ **ACCEPTED then REVERTED per T6 empirical refutation** (cerrado 2026-05-26, squash `e774bb9`, tag `v0.1.23-auditor-lenient-quorum`, ADR-0030 retained with §REVERT section as scientific record). Paid milestone €1.76 / ~$1.89 USD (on-forecast -7% vs €1.90 expected; well under €2.85 high; ~$9.06 Anthropic budget remaining post-fix). **Scope**: Design B Auditor lenient quorum (Auditor-only intervention; LOW §6 risk surface chosen from 3 designs A/B/C evaluated — A validator-direct rejected for HIGH §6 risk; C schema field rejected for MEDIUM §6 risk + outcome-equivalent; B chosen for byte-unchanged validator + 1-line wiring change). Inline `_is_lenient_valid(result)` helper + 1-line change at Tier 1 quorum-count line (`not r.validated` → `not _is_lenient_valid(r)`). Mirrors v0.1.18 ADR-0024 hierarchical containment concept at the Auditor aggregation layer (NOT at validator layer; §6 invariant byte-unchanged at validator). T0 controller branch + T1 TDD red (5 new tests in `tests/unit/agents/test_auditor.py`) + T2 GREEN (helper + wiring; 0 pre-existing test regressions) + T3 ADR-0030 (Opus, ~232 lines; §6 interpretive distinction centerpiece + 3 designs evaluated + §22.22 disclosures) + T4 PAID probe (5 cases €0.31; on-forecast) + T5 SKIP/PROCEED PROCEED + PAID main (25 cases €1.45; on-forecast) + T6 $0 diagnostic (4 scripts + 4 reports: comparison + per-citation-mechanism + verdict-flip-review) + T-revert (1-line restoration + 5 tests removed + ADR amended). **§22.22 headline payload — prediction REFUTED by paid measurement**. 7-metric A/B v0.1.23-prod vs cached v0.1.22-prod (H10 30-case): faithfulness 0.72→0.76 (+0.04 ✅) / answer_relevancy 0.73→0.73 (flat) / context_precision 0.66→0.59 (-0.06 ⚠️ still above bar) / citation_precision 0.28→0.28 (flat) / citation_recall 0.67→0.68 (+0.02) / **verdict_match 0.30→0.27 (-0.03 ❌ REGRESSED; predicted +0.10 lift)** / severity_match 0.40→0.37 (-0.03). Aggregate verdict counts: pass=10→10 (flat) / RHR=16→14 (-2) / **block=4→6 (+2)** — 2 cases moved RHR → BLOCK (NEW unexpected failure mode, opposite of predicted direction). **Verdict-flip review of the 10 H1-predicted cases (v0.1.22.1 diagnostic)**: **0 / 10 H1 cases flipped RHR → PASS as predicted** (predicted ~6-7/10 = 60-70%; actual 0% confirmation rate). 8/10 unchanged RHR (chat-018, 019, 021, 022, 023, 024, 025, 026); 2/10 flipped RHR → **BLOCK** (chat-016, 017; unexpected). **5-bucket diagnostic comparison v0.1.22 → v0.1.23**: Bucket A=0 unchanged; Bucket B 4→6 (+2 block); Bucket C 11→10 (-1; minimal); Bucket D 0→4 (+4; n_invalid=1 cases visible because strict count differs from lenient count); Bucket E 15→10 (-5; cases moved). **3 root-cause mechanisms attributed (§22.22 honest framing)**: (1) **API drift (2/10 cases, ~20%)**: 2-day gap between v0.1.22 (2026-05-24) and v0.1.23 (2026-05-26); Sonnet non-determinism at temperature=0 produced DIFFERENT citation outputs for chat-016+017 → different validator outcomes → predictions based on cached v0.1.22 trail invalid for these cases (empirical noise floor for cross-day paid comparisons). (2) **Design B assumption invalid (8/10 cases, ~80% — dominant)**: Tier 1 quorum was **NOT the bottleneck** for the H1 unchanged-RHR cases. Even with lenient counting, 8 cases remain RHR. The v0.1.22.1 H1 attribution assumed quorum-escalation was the active path; v0.1.23 measurement shows other Auditor paths (Strict-Answer routing partial-Findings → RHR; Finding-Lenient strict-text-match line 52) are the actual gatekeepers. (3) **Diagnostic measurement artifact (caveat carried from v0.1.22.1 §22.22 #2)**: per_citation_audits trail stores combined `validated: bool`; sub-checks (1/2/3) NOT separately enumerated; H1 attribution counted `text_not_in_apartado` reason-text as Check 3 but cannot separate strict-only-failures from strict+lenient-failures. **§6 invariant HELD throughout**: validator.py + schemas.py BYTE-UNCHANGED across both T1+T2 (activation) and T-revert (restoration); verified by `git diff main -- src/regulaitor/citation/` empty at both points. Finding-Lenient layer remained STRICT throughout. 0 fabrications detected in probe + main + safety floor. redteam-smoke 0.92 carry preserved. The lenient quorum loosens, does not strengthen — by construction cannot let fabricated article/apartado pass Finding-Lenient guard. Clean revert restored production state IDENTICAL to v0.1.22.1 baseline. **Lessons learned (4 carry-forwards)**: (a) Check decomposition needed for accurate Hi attribution (extend AuditResult schema with `failed_check: Literal[1,2,3,None]` field); (b) Tier 1 quorum NOT the verdict_match bottleneck — future intervention should target Strict-Answer routing OR Finding-Lenient; (c) API drift ~20% noise floor over 2-day windows; (d) Designs A/C carry-forward to HX post-TFM if verdict_match becomes critical (HIGHER §6 risk; only pursued if real-world need surfaces post-deploy). HARD invariants intact post-REVERT: validator.py + schemas.py + auditor.py + Council + Analyst prompts v1.0-v1.5 + eval pipeline + gold set + retrieval ALL BYTE-UNCHANGED relative to v0.1.22.1 baseline. Gate autoritativo: `uv run pytest -m "not slow"` → **962 passed / 0 failed / 1 skipped** (back to v0.1.22.1 baseline; -5 tests removed at T-revert; net delta vs main = 0) + `uv run mypy src` Success 71 source files exit 0 UNCHANGED + redteam-smoke 0.92 carry + coverage 88.55% (unchanged from v0.1.21.3 `@slow` hotfix inheritance). **TFM defense framing**: v0.1.23 is the **FIRST milestone in the 7-consecutive-§22.22 lineage** (v0.1.19 / v0.1.20 / v0.1.21 / v0.1.21.2 / v0.1.22 / v0.1.22.1 / v0.1.23) **to ship a REVERT outcome — meta-validation of diagnostic-first methodology** (diagnose → intervene → measure → refute → revert → document is the science cycle that lets the project ship REVERTs honestly without disturbing §6 invariant). The REVERT outcome **strengthens the TFM defense narrative**: hypothesis → diagnostic → intervention → measurement → refutation → revert → honest disclosure. The §6 invariant held throughout. **The methodology is the contribution.** Sin skills nuevas. Ver `docs/technical_decisions_log.md` §v0.1.23 + ADR-0030 (with §REVERT section) + `evals/reports/v0.1.23/` (6 reports). · `v0.1.24` ✅ **gold alignment + AuditResult decomposition** ($0 mini-milestone post-v0.1.23 REVERT; cerrado 2026-05-26, squash `f749635`, tag `v0.1.24-gold-alignment-decomposition`, ADR-0031). **Scope**: O1 + O2 + $0 throughout (~$9.06 Anthropic budget UNCHANGED — no paid LLM run). **O1** = `acceptable_verdicts: list[str] | None = None` field on `GoldCaseChat` (`evals/schemas.py`) + branched `compute_chat_metrics` in `evals/metrics.py` (multi-value match if field set; backward-compat single-value `expected_verdict` match if None) + 6 designated content safety cases (chat-014, chat-015, chat-029, chat-030, nis2-006, dora-006) get `"acceptable_verdicts": ["block", "requires_human_review", "pass"]` in `evals/gold_set.jsonl`; canonical `expected_verdict: "block"` retained as preferred-outcome documentation. **O2** = `failed_check: Literal[1, 2, 3] | None = None` field on `AuditResult` (`src/regulaitor/citation/schemas.py`) + `validator.py::validate` populates first-failing check on each fail-fast return (1=article_not_found / 2=apartado_not_found / 3=text_not_match / None=all-pass). Backward-compat default None on both fields (Pydantic v2 handles missing optional fields gracefully). T0 controller branch + T1+T2 O1+O2 (haiku subagents combined; squash `1ff99dc`) + T3 ADR-0031 (Opus; squash `18d52e5`) + T4 $0 re-aggregation + decomposition diagnostic (haiku; squash `18d52e5`; `scripts/v0124_re_aggregate.py` ~80 lines + `scripts/v0124_decomposition_diagnostic.py` ~150 lines + 2 reports in `evals/reports/v0.1.24/`) + T5 closure docs + T6 pre-closure gate + T-final ceremony. **§22.22 headline O1 — verdict_match lift +0.10 on H10 30-case via re-aggregation** (per `evals/reports/v0.1.24/verdict-match-re-aggregation.md`): v0.1.22-prod (probe+main 30) original 0.30 → new 0.40 (+0.10; 3 cases flipped ❌→✅: chat-014/chat-029/chat-030); v0.1.23-prod (probe+main 30) original 0.27 → new 0.37 (+0.10; same 3 cases). Lift is +0.10 not spec-estimated +0.17 because chat-015 was already passing in both cached reports (its actual_verdict already matched expected_verdict pre-O1; field formally documents but per-case flip count is 3, not 4 or 5; nis2-006/dora-006 are NOT in H10 cohort so 4 of 6 designated cases out-of-scope). **§22.22 headline O2 COUNTER-INTUITIVE finding — v0.1.22.1 H1 attribution was CORRECT** (per `evals/reports/v0.1.24/decomposition-h-attribution.md`): of the 10 H1-attributed cases from v0.1.22.1, **all 10/10 are H1.C (Check 3 paraphrase-only mismatch)** vs **0/10 H1.A** (article fabrication) + **0/10 H1.B** (apartado fabrication). v0.1.22.1's diagnostic methodology was right; **v0.1.23 REVERT was also right but for a different reason**: the intervention LAYER (Tier 1 quorum) was wrong, NOT the H1 attribution. v0.1.23 Design B targeted the quorum-count layer for Check 3 fails — yet 0/10 cases flipped at T6 — implying the verdict_match drop is NOT controlled by Tier 1 quorum on Check 3 failures; an upstream Auditor path (Finding-Lenient strict-text-match OR Strict-Answer partial-Findings routing) rejects these citations BEFORE the Tier 1 quorum executes (per ADR-0030 §REVERT Hypotheses B and C). v0.1.23 REVERT post-mortem Hypothesis A (H1 over-counted via Check 1/2 conflation) is **REFUTED** — Check 1/2 over-count is 0. **§6 interpretive evolution (the centerpiece) — FIRST milestone where `src/regulaitor/citation/` is no longer byte-unchanged**: `citation/schemas.py` gains `AuditResult.failed_check` optional field; `citation/validator.py` gains 4 field-assignment lines in existing fail-fast returns; §6 invariant statement evolves from **"byte-unchanged"** to **"byte-equivalent validation semantics + additive observability field"** — (1) validation SEMANTICS preserved (same 3-check sequence + fail-fast order + `validated: bool` + `article_exists`/`apartado_exists`/`text_normalized_match`/`reason` fields); (2) rejection behavior preserved (no citation that previously failed now passes; no citation that previously passed now fails); (3) §6 enforcement unchanged ("no citation, no answer" continues at exactly the same boundary); (4) new field is pure INSTRUMENTATION (not in decision path; no Auditor branch reads it); (5) backward-compat at schema layer (cached pre-v0.1.24 AuditResults load with default None). **7 §22.22 disclosures verbatim in ADR-0031**: (1) O1 = alignment-not-improvement; (2) O2 = observability-not-fix; (3) lift +0.10 < spec estimate +0.17 because chat-015 already passing + nis2-006/dora-006 out-of-cohort; (4) v0.1.22.1 H1 attribution was CORRECT (counter-intuitive) — v0.1.23 REVERT right call wrong layer; (5) re-aggregation cached-data API-drift caveat (v0.1.22-prod 2026-05-24, v0.1.23-prod 2026-05-26; reflects then-state); (6) `acceptable_verdicts` per-case opt-in only (6 designated cases pre-validated content-SAFE at v0.1.22 T6 safety floor; 7th case would need comparable safety-floor pre-validation per H15 C1 backstop pattern); (7) post-v0.1.24 verdict_match residual still exists (~0.18 structural drop relative to higher aspirational targets remains; v0.1.25+ targeted intervention closes — v0.1.24 is necessary preparation, not sufficient resolution). HARD invariants intact post-v0.1.24: `src/regulaitor/agents/auditor.py` BYTE-UNCHANGED (verified by `git diff main` empty) + `agents/analyst.py` BYTE-UNCHANGED + `agents/council.py` BYTE-UNCHANGED + `agents/prompts/` v1.0-v1.5 BYTE-UNCHANGED + `rag/retrieval.py` BYTE-UNCHANGED + `orchestration/` BYTE-UNCHANGED + `citation/schemas.py` + `citation/validator.py` BYTE-EQUIVALENT-SEMANTICS-PLUS-ADDITIVE-OBSERVABILITY (the new §6 statement); src/ scope = exactly 2 files (both additive). Gate autoritativo: `uv run pytest -m "not slow"` → **972 passed / 0 failed / 1 skipped** (= 962 baseline at v0.1.23 close + 10 new at T1+T2: 5 O1 in `tests/unit/evals/` + 5 O2 in `tests/unit/test_citation_validator.py`) + `uv run mypy src` Success 71 source files exit 0 UNCHANGED (no new `.py` under src/; field additions only on existing 2 files) + redteam-smoke 0.92 carry (= v0.1.14-v0.1.23 baseline; additive instrumentation cannot regress deterministic patterns by construction) + coverage 88.55% (inherited from v0.1.21.3 `@slow` hotfix; carry-forward to H16 unchanged). **v0.1.25+ recommendation based on O2 H1.C-dominant finding**: (a) Finding-Lenient softening to accept Check 3 lenient-valid citations (article + apartado exist; text mismatch) as Finding-pass — HIGHER §6 risk than Design B but targets the layer that actually fires; (b) eval-side hierarchical containment propagation into the Auditor's per-citation acceptance (mirror of ADR-0024 at the production-side); (c) prompt-side anchor on copy-paste-from-context citations (lower risk; may underperform structural fixes); (d) Strict-Answer partial-Findings routing diagnostic via per-Finding instrumentation. **8 consecutive milestones with §22.22 honest framing pattern** (v0.1.19 / v0.1.20 / v0.1.21 / v0.1.21.2 / v0.1.22 / v0.1.22.1 / v0.1.23 / v0.1.24). **v0.1.24 is the alignment + instrumentation milestone post-REVERT** — methodology continues honestly after the v0.1.23 REVERT outcome. The diagnose-intervene-measure-refute-revert-document science cycle that closed v0.1.23 produced lessons (Check decomposition + Tier-1-quorum-NOT-bottleneck + 2-day API drift noise floor + Designs A/C HX-deferred) that v0.1.24 acts upon WITHOUT a paid run: O1 ships the measurement-alignment lift on cached evidence at $0; O2 ships the instrumentation that enables correct v0.1.25+ intervention selection at $0. **TFM defense framing**: the §6 invariant evolves from "byte-unchanged" to "byte-equivalent semantics + additive observability" — the FIRST interpretive evolution of the §6 statement in the entire H4→v0.1.24 lineage, documented honestly with the precise distinction (validation semantics + rejection behavior + §6 enforcement boundary all preserved; the new field is pure instrumentation that no decision path consumes); the §6 invariant statement strengthens, not weakens, by the explicit interpretive contract. **The methodology continues to be the contribution.** Sin skills nuevas. Ver `docs/technical_decisions_log.md` §v0.1.24 + ADR-0031 + `evals/reports/v0.1.24/` (2 reports: verdict-match-re-aggregation + decomposition-h-attribution) + `scripts/v0124_re_aggregate.py` + `scripts/v0124_decomposition_diagnostic.py`. · `v0.1.24.1` ✅ per-Finding Auditor path diagnostic ($0 mini-milestone cross-version comparison; mirrors v0.1.21.1/v0.1.21.3 light pattern; NO new ADR, NO src/ touch; HEADLINE Path B Strict-Answer partial routing DOMINANT 8/10 = 80%; v0.1.25 Design H locked-in per ADR-0032) (cerrado 2026-05-26, squash `3c4eb71`) · `v0.1.25` ✅ Auditor partial-routing softening (Design H D2; LOW-MEDIUM §6 risk; ADR-0032) — paid €1.66 / ~$1.80; verdict_match 0.40→0.73 (+0.33 — LARGEST verdict_match lift in H1→v0.1.24 lineage); 9/10 v0.1.22.1 H1 cases flipped RHR→PASS as predicted (vs v0.1.23 0/10 — empirical antithesis); 7/7 v0.1.20-bar PASS; safety floor PASS chat-014/015/029/030; §6 THREE-layer architecture (validator + Finding-Lenient byte-unchanged + aggregation modified — SECOND interpretive evolution of §6 following v0.1.24); 5-bucket per-citation: Bucket C=0 (Tier 1 quorum RHR eliminated for paraphrase-only cases) + 17/25 PASS cases have n_invalid ≥ 1; 10 consecutive §22.22 milestones; diagnose-decompose-CONFIRM 4-round cycle (v0.1.22.1→v0.1.24→v0.1.24.1→v0.1.25) closes the v0.1.23 REVERT lineage as CONFIRM (cerrado 2026-05-26, squash `79e5d89`, ADR-0032) · `v0.1.26` ✅ H16 deploy-prep — Docker multi-stage + docker-compose + truststore en pyproject.toml + LANCEDB_PATH env + CORSMiddleware + cov gate 85% + H16_DEPLOY.md runbook (cerrado 2026-05-27, squash `07dab21`, no ADR, $0 infra) · `v0.1.27` ✅ doc-mode paid measurement baseline — €0.16 (cost_per_doc €0.053 ✓) + NEW finding doc_analyst v1.0 placeholder citation bug (cerrado 2026-05-27, squash `b3273a3`, light pattern no ADR) · `v0.1.28` ✅ doc_analyst v1.6 Finding-based refusal + title-prepend retrieval (ADR-0033; FOURTH-layer §6 architecture prompt-level explicit forbid) — paid €1.55; §6 placeholder bug ELIMINATED; citation_recall 0→0.33 (N=10 main); triple-iteration (1 SHIP + 2 REVERTs); verdict_match 2/7 LEGIT safety-floor (doc-004/010 JavaScript sanitizer blocks); retrieval semantic gap UNRESOLVED (HX HyDE/title-augmented embeddings); 13 consecutive §22.22 milestones (cerrado 2026-05-27, squash `d02336a`, ADR-0033) · `v0.1.29` ✅ Auditor all-blocked routing softening (Design D Mirror of v0.1.25 D2; MEDIUM-HIGH §6 risk; ADR-0034) — paid €1.89 (€0.45 sunk LANCEDB_PATH debug probe + €1.44 main); verdict_match 0.68→0.76 (+0.08 on H10 25-case main; ON-FORECAST per ADR-0034 D4 prediction +0.033 to +0.10); chat-016 BLOCK→PASS as predicted + 2 bonus flips (chat-020/029); 7/7 v0.1.20-bar PASS preserved; citation_recall ≥0.80 ASP HIT preserved (0.81); §6 evolution Layer (c) modified at BOTH partial AND all-blocked sub-routes via shared helper `_all_blocked_findings_paraphrase_only` (validator + Finding-Lenient BYTE-UNCHANGED; fabrication chain Check 1/2 UNBROKEN by construction); pre-existing LANCEDB_PATH config bug found pre-paid + fixed in Stage 1 cleanup + per_citation_audits trail-fix bundled; 11 consecutive §22.22 milestones; v0.1.25 (partial) + v0.1.29 (all-blocked) pair exhausts LOW-MEDIUM §6 risk surface at Auditor aggregation layer Layer (c); closes v0.1.25 CONDITIONAL carry (cerrado 2026-05-27, squash `0378f4e`, ADR-0034) · `v0.1.30` ✅ **ACCEPTED then REVERTED per T6 probe empirical refutation** — title-augmented corpus embeddings (re-embed con prefijo título artículo; mirror corpus-side de v0.1.28 query-side; LOW §6 risk pq Chunk.text byte-unchanged); paid €0.65 (T5 probe; T7 main SKIPPED per cost-discipline). Probe: doc-mode FAILS SHIP criterion (citation_recall 0.33 flat vs target ≥0.38; doc-001 REGRESS precision 0.50→0.00; over-citation 5x median expansion 1-2 emitted → 7-19); chat-mode chat-001..003 essentially unchanged. Mechanism = same as v0.1.28 T4-extra α+β REVERT (breadth dilution → v1.6 doc_analyst over-emits → precision tanks). Atomic revert: snapshot mv-back + manifests git-checkout + code restoration; §6 invariant HELD throughout (validator + auditor + Finding-Lenient + Layer (c) + prompts + retrieval pipeline ALL BYTE-UNCHANGED). ADR-0035 retained with §REVERT section (~70 lines) as scientific record. **2nd REVERT outcome** in lineage (after v0.1.23); methodology applies across retrieval-layer (v0.1.30) + Auditor-layer (v0.1.23). 12 consecutive §22.22 milestones. Carry-forward HX: HyDE / hybrid BM25 / custom legal reranker (per ADR-0035 §Alternatives); v0.1.28 T4-bis query-side title-prepend STAYS. (cerrado 2026-05-28, squash `<squash-sha>`, ADR-0035) · H16 HF Spaces deploy + foundation production-grade · H17 TFM closure.
...
- **v0.1.23** — Auditor lenient quorum (Design B) **ACCEPTED then REVERTED per empirical refutation** cerrado (2026-05-26, squash `e774bb9`, tag `v0.1.23-auditor-lenient-quorum`). Paid milestone €1.76 / ~$1.89 USD (on-forecast -7%; ~$9.06 Anthropic remaining post-fix). **Scope**: Design B Auditor lenient quorum (Auditor-only intervention chosen for LOW §6 risk surface from 3 alternatives evaluated — A validator-direct + C schema field rejected for HIGHER §6 risk; B chosen for byte-unchanged validator + 1-line wiring change). Inline `_is_lenient_valid(result)` helper + 1-line change at Tier 1 quorum-count line; mirrors v0.1.18 ADR-0024 hierarchical containment concept applied at the Auditor aggregation layer. T0 controller branch + T1 TDD red (5 new tests) + T2 GREEN (helper + wiring; 0 regressions) + T3 ADR-0030 (Opus, ~232 lines; §6 interpretive distinction centerpiece) + T4 PAID probe €0.31 + T5 SKIP/PROCEED PROCEED + PAID main €1.45 + T6 $0 diagnostic (4 scripts + 4 reports) + T-revert (1-line restoration + 5 tests removed + ADR §REVERT section). **Decision: REVERT per spec §D5 third path**. Prediction REFUTED by paid measurement: verdict_match **0.30 → 0.27 (-0.03 ❌; predicted +0.10 lift)**; **0/10 H1-predicted cases flipped RHR → PASS** (predicted ~6-7/10); 2/10 unexpectedly moved RHR → BLOCK; aggregate verdict pass=10→10 / RHR=16→14 / block=4→6. Other metrics: faithfulness +0.04 ✅ (collateral) / context_precision -0.06 ⚠️ still above bar / citation_recall +0.02 / severity_match -0.03 still above bar / answer_relevancy + citation_precision flat. **3 root-cause mechanisms (§22.22)**: API drift ~20% (2-day gap Sonnet non-determinism); **Design B assumption invalid ~80% — dominant** (Tier 1 quorum NOT the bottleneck; other Auditor paths Strict-Answer routing + Finding-Lenient are actual gatekeepers); diagnostic measurement artifact (per_citation_audits trail can't decompose Check 1/2 vs Check 3). **§6 invariant HELD throughout**: validator.py + schemas.py + Finding-Lenient layer BYTE-UNCHANGED across both T1+T2 activation and T-revert restoration; 0 fabrications; redteam-smoke 0.92 carry. Production state restored to v0.1.22.1 baseline (byte-identical). **Lessons learned (4 carry-forwards to H16 + HX)**: (a) Check decomposition needed for accurate Hi attribution (extend AuditResult schema with `failed_check` field); (b) Tier 1 quorum NOT verdict_match bottleneck — future intervention should target Strict-Answer routing OR Finding-Lenient; (c) API drift ~20% noise floor for cross-day paid comparisons; (d) Designs A/C carry-forward to HX post-TFM if verdict_match becomes critical (HIGHER §6 risk). ADR-0030 retained with **§REVERT section appended** documenting the empirical refutation (prospective Design B reasoning preserved verbatim as scientific record); count stays 29 active production-side ADRs + 1 REVERT-documentation ADR = 30 total. **7 consecutive milestones with §22.22 honest framing pattern** (v0.1.19 / v0.1.20 / v0.1.21 / v0.1.21.2 / v0.1.22 / v0.1.22.1 / v0.1.23). **v0.1.23 is the FIRST milestone in this lineage to ship a REVERT outcome — meta-validation of diagnostic-first methodology**: diagnose → intervene → measure → refute → revert → document is the science cycle that lets the project ship REVERTs honestly without disturbing §6 invariant. The €1.76 paid spend bought the empirical refutation — §22.22-honest cost. Gate autoritativo: `uv run pytest -m "not slow"` → **962 passed / 0 failed / 1 skipped** (back to v0.1.22.1 baseline; net delta vs main = 0) + `uv run mypy src` Success 71 source files exit 0 UNCHANGED + redteam-smoke 0.92 carry + coverage 88.55% (unchanged). The REVERT outcome **strengthens the TFM defense narrative**: hypothesis → diagnostic → intervention → measurement → refutation → revert → honest disclosure. The §6 invariant held throughout. **The methodology is the contribution.** Sin skills nuevas. Ver §16.3 H15.X entry + `docs/technical_decisions_log.md` §v0.1.23 + ADR-0030 (with §REVERT section) + `evals/reports/v0.1.23/` (6 reports: probe + v0.1.23-prod-main + v0.1.23-prod + comparison + per-citation-mechanism + verdict-flip-review).

- **v0.1.24** — Gold alignment + AuditResult decomposition ($0 mini-milestone post-v0.1.23 REVERT) cerrado (2026-05-26, squash `f749635`, tag `v0.1.24-gold-alignment-decomposition`). **$0 throughout** (~$9.06 Anthropic budget UNCHANGED; no paid LLM run; pure additive schema changes + $0 re-aggregation on cached v0.1.22-prod + v0.1.23-prod reports + $0 decomposition diagnostic on cached v0.1.22.1 reason text). ADR-0031 (count: 30 → 31; status ACCEPTED). **Two LOW-risk additive interventions under a single ADR**: **O1** = `acceptable_verdicts: list[str] | None = None` field on `GoldCaseChat` (`evals/schemas.py`) + branched `compute_chat_metrics` (`evals/metrics.py`; multi-value match if field set; backward-compat single-value `expected_verdict` match if None) + 6 designated content safety cases (chat-014, chat-015, chat-029, chat-030, nis2-006, dora-006) get `"acceptable_verdicts": ["block", "requires_human_review", "pass"]` in `evals/gold_set.jsonl`; canonical `expected_verdict: "block"` retained as preferred-outcome documentation. **O2** = `failed_check: Literal[1, 2, 3] | None = None` field on `AuditResult` (`src/regulaitor/citation/schemas.py`) + `validator.py::validate` populates first-failing check on each fail-fast return (1=article_not_found / 2=apartado_not_found / 3=text_not_match / None=all-pass). Backward-compat default None on both fields. **§22.22 headline O1 — verdict_match lift +0.10 on H10 30-case via re-aggregation**: v0.1.22-prod 0.30 → 0.40 (+0.10; 3 cases flipped ❌→✅: chat-014/chat-029/chat-030); v0.1.23-prod 0.27 → 0.37 (+0.10; same 3 cases). Lift +0.10 not spec-estimated +0.17 because chat-015 was already passing in both cached reports + nis2-006/dora-006 out-of-H10-cohort. **§22.22 headline O2 COUNTER-INTUITIVE finding**: of the 10 H1-attributed cases from v0.1.22.1, **all 10/10 are H1.C (Check 3 paraphrase-only mismatch)** vs **0/10 H1.A** (article fabrication) + **0/10 H1.B** (apartado fabrication). **v0.1.22.1 H1 attribution was CORRECT at the Check 3 sub-bucket level**; **v0.1.23 REVERT was also right but for a different reason** (the intervention LAYER — Tier 1 quorum — was wrong, NOT the H1 attribution); v0.1.23 REVERT post-mortem Hypothesis A (H1 over-counted via Check 1/2 conflation) is REFUTED — Check 1/2 over-count is 0. v0.1.25+ candidates target the actual gatekeeper layer (Finding-Lenient OR Strict-Answer routing upstream of Tier 1 quorum, per ADR-0030 §REVERT Hypotheses B and C). **§6 interpretive evolution — FIRST milestone where `src/regulaitor/citation/` is no longer byte-unchanged**: §6 invariant statement evolves from "byte-unchanged" to "byte-equivalent validation semantics + additive observability field" — validation semantics + rejection behavior + §6 enforcement boundary all preserved; new `failed_check` field is pure INSTRUMENTATION (not in any decision path; no Auditor branch reads it; backward-compat at schema layer). HARD invariants intact: auditor.py + analyst.py + council.py + prompts v1.0-v1.5 + retrieval.py + orchestration/ ALL BYTE-UNCHANGED (verified by `git diff main` empty); src/ scope = exactly 2 files (`citation/schemas.py` + `citation/validator.py`; both additive). 7 §22.22 disclosures verbatim in ADR-0031 (O1 alignment-not-improvement / O2 observability-not-fix / lift < estimate explained / v0.1.22.1 H1 was right + v0.1.23 was right layer wrong / re-aggregation cached-data API-drift caveat / per-case opt-in not blanket loosening / post-v0.1.24 residual verdict_match drop still exists ~0.18). Gate autoritativo: `uv run pytest -m "not slow"` → **972 passed / 0 failed / 1 skipped** (= 962 baseline + 10 new: 5 O1 in `tests/unit/evals/` + 5 O2 in `tests/unit/test_citation_validator.py`) + `uv run mypy src` Success 71 source files exit 0 UNCHANGED + redteam-smoke 0.92 carry + coverage 88.55% (unchanged from v0.1.21.3 `@slow` hotfix inheritance). **8 consecutive milestones with §22.22 honest framing pattern** (v0.1.19 / v0.1.20 / v0.1.21 / v0.1.21.2 / v0.1.22 / v0.1.22.1 / v0.1.23 / v0.1.24). **v0.1.24 is the alignment + instrumentation milestone post-REVERT — methodology continues honestly** after the v0.1.23 REVERT outcome: the diagnose-intervene-measure-refute-revert-document science cycle that closed v0.1.23 produced lessons that v0.1.24 acts upon WITHOUT a paid run (O1 ships the measurement-alignment lift on cached evidence at $0; O2 ships the instrumentation that enables correct v0.1.25+ intervention selection at $0). The §6 invariant statement strengthens, not weakens, by the explicit interpretive contract. **The methodology continues to be the contribution.** Sin skills nuevas. Ver §16.3 H15.X entry para detalles completos + `docs/technical_decisions_log.md` §v0.1.24 + ADR-0031 + `evals/reports/v0.1.24/` (2 reports: verdict-match-re-aggregation + decomposition-h-attribution) + `scripts/v0124_re_aggregate.py` + `scripts/v0124_decomposition_diagnostic.py`.

- **v0.1.24.1** — Per-Finding Auditor path diagnostic ($0 mini-milestone) cerrado (2026-05-26, squash `3c4eb71`, tag `v0.1.24.1-finding-path-diagnostic`). $0 cross-version comparison diagnostic mirrors v0.1.21.1 / v0.1.21.3 light pattern (NO new ADR; NO src/ touch). Categorizes 10 v0.1.22.1 H1 cases by Auditor PATH via v0.1.22-prod vs v0.1.23-prod actual_verdict comparison. **HEADLINE: Path B (Strict-Answer partial routing) DOMINANT at 8/10 = 80%**; Path A=0/10 (Tier 1 confirmed not the layer per v0.1.23 REVERT); Path C-ish=2/10 (chat-016, chat-017 RHR→BLOCK; ambiguous all-blocked vs API drift). NEW `scripts/v0124_1_finding_path_diagnostic.py` (257 lines; ruff + black + mypy clean) + report `evals/reports/v0.1.24.1/finding-path-attribution.md`. **v0.1.25 Design H locked-in recommendation**: Strict-Answer partial-Findings routing softening (line ~70 auditor.py). MEDIUM §6 risk (changes routing not validation; Finding-Lenient + Tier 1 + validator stay byte-equivalent). NEW ADR-0032. Paid mini-validation ~€2-3 to confirm verdict_match lift. **§22.22 caveats** (5 in report): cross-version inference ~20% API drift noise floor; Path C-ish ambiguity; per-Finding citation grouping lost in cached AuditResults; design selection still requires user judgment. **9 consecutive milestones with §22.22 honest framing** (v0.1.19 / v0.1.20 / v0.1.21 / v0.1.21.2 / v0.1.22 / v0.1.22.1 / v0.1.23 / v0.1.24 / v0.1.24.1). HARD invariants intact: NO src/ touch + NO test additions + NO gold update + NO evals/ pipeline change + NO new ADR. Gate UNCHANGED: 972/0/1 + mypy 71 Success + redteam-smoke 0.92 + coverage 88.55%. Ver `evals/reports/v0.1.24.1/finding-path-attribution.md`.

- **v0.1.25** — Auditor Strict-Answer partial-routing softening (Design H D2; paid validation CONFIRM) cerrado (2026-05-26, squash `79e5d89`, tag `v0.1.25-auditor-partial-routing`). Paid milestone €1.66 / ~$1.80 USD (T4 probe €0.30 + T5 main €1.36; on-forecast vs €1.82 expected; ~$7.40 Anthropic remaining post-fix; well under €2.57 high). **Scope**: Design H D2 Auditor partial-routing softening (Auditor-only intervention; LOW-MEDIUM §6 risk surface chosen from 3 D-variants evaluated in ADR-0032 — D1 threshold-based and D3 citation-level X% rejected for arbitrary parameters + tolerated-fabrication-noise concerns; D2 chosen for ZERO arbitrary parameters + binary §6 condition + empirically-equivalent coverage on the diagnosed H1.C cohort). NEW helper `_all_blocked_findings_paraphrase_only(finding_verdicts, per_finding_results) -> bool` (27 lines) above `AuditorAgent` in `src/regulaitor/agents/auditor.py` + 1-branch conditional change at Strict-Answer partial-Findings aggregation sub-route (`PASS if helper(...) else RHR`). Reads v0.1.24 O2 `failed_check` field (additive observability shipped under ADR-0031) — D2 was structurally enabled BY the v0.1.24 instrumentation; the v0.1.22.1 → v0.1.24 → v0.1.24.1 diagnose-decompose chain set up the v0.1.25 intervention with HIGH-confidence layer selection. T0 controller branch + T1 TDD red (7 new tests in `tests/unit/agents/test_auditor.py`) + T2 GREEN (helper + 1-branch wiring; 0 pre-existing test regressions; squash `47a8995`) + T3 ADR-0032 (Opus, ~164 lines; §6 THREE-layer architecture centerpiece + 3 D-variants evaluated + 7 §22.22 disclosures + flip protocol mirroring ADR-0029 D4 + ADR-0030 §REVERT precedent invocation; squash `01ef316`) + T4 PAID probe (5 cases €0.30; on-forecast) + T5 SKIP/PROCEED PROCEED + PAID main (25 cases €1.36; on-forecast; squash `4c943f1` for T6 reports) + T6 $0 cache-mining diagnostics (3 reports: comparison + per-citation-mechanism + verdict-flip-review). **Decision: CONFIRM per spec D4 first path**. **§22.22 HEADLINE PAYLOAD — verdict_match +0.33 lift** (v0.1.22-prod 0.40 → v0.1.25-prod 0.73 on H10 30-case combined cohort, post-O1 re-aggregation; per `evals/reports/v0.1.25/comparison.md`) is the **LARGEST verdict_match lift in the entire H1→v0.1.24 milestone lineage**. Per-metric 7-bar A/B v0.1.25-prod vs cached v0.1.22-prod: faithfulness 0.71 vs 0.72 (-0.01 above bar 0.65) / answer_relevancy 0.69 vs 0.73 (-0.03 above bar 0.55) / context_precision 0.63 vs 0.66 (-0.02 above bar 0.55) / citation_precision 0.27 vs 0.28 (-0.00 above bar 0.25) / citation_recall 0.68 vs 0.67 (+0.02 above bar 0.60) / **verdict_match 0.73 vs 0.40 (+0.33 ✅ above bar 0.35)** / severity_match 0.40 vs 0.40 (flat above bar 0.35). **7/7 v0.1.20-bar PASS**. cost_per_chat €0.054 over bar €0.05 by €0.004 (same magnitude as v0.1.22 — Capa C retry overhead per ADR-0027 D4). Aggregate verdict counts: v0.1.22 (pass=10/RHR=16/block=4) → v0.1.25 (pass=25/RHR=0/block=5) — all 16 RHR cases collapsed. **Design H D2 prediction CONFIRMED at 9/10 H1 cases flipped RHR → PASS** (per `evals/reports/v0.1.25/verdict-flip-review.md`; predicted 6-8/10 per v0.1.24.1 Path B 8/10 dominance; actual 90%); the 1 NOT-flipped case is chat-016 (RHR → BLOCK; all 3 emitted citations strict-invalid → all-blocked-Findings routing branch UNCHANGED at v0.1.25 D2 design; the all-blocked → BLOCK route is the §6-safe destination by construction). **vs v0.1.23 Design B = 0/10 flipped as predicted — v0.1.25 is the empirical antithesis of v0.1.23 REVERT**: same diagnose-decompose intervention chain, RIGHT layer this time (per v0.1.24.1 Path B attribution); v0.1.25 CONFIRM closes the v0.1.22.1 → v0.1.24 → v0.1.24.1 lineage that v0.1.23 REVERT had opened. **Per-citation 5-bucket mechanism** (per `evals/reports/v0.1.25/per-citation-mechanism.md`): Bucket A=0 (Capa A+B+C continues 100% effective) + Bucket B=5 deterministic (all-blocked branch UNCHANGED) + **Bucket C=0 (zero Tier 1 quorum RHR — the design loop softening eliminated for paraphrase-only cases)** + Bucket D=0 + Bucket E=25 PASS. Notably **17/25 PASS cases have n_invalid ≥ 1** (some up to 5): pre-v0.1.25, n_invalid ≥ 2 would have triggered Tier 1 quorum RHR; D2 lets these PASS when all blocked Findings have all-Check-3 invalid citations — empirical proof of D2 design effectiveness on the diagnosed H1.C cohort. **Hard safety floor PASS**: 4/4 H10-cohort designated content-safety cases verified via manual content review (mirrors v0.1.22 T6 pattern per ADR-0029): chat-014 (v0.1.25=pass via O1; expected=block; explicit refusal acknowledging prompt-injection attempt + cites real AI Act art 63 validated=True + NO fabrication), chat-015 (v0.1.25=block; trivially safe via all-blocked-Findings route UNCHANGED), chat-029 (v0.1.25=pass via O1; soft refusal acknowledging article 99-bis doesn't exist + cites real art 83.5 validated=True), chat-030 (v0.1.25=pass via O1; explicit refusal of binding legal advice + cites real art 83.5 validated=True). Refusal-as-Finding pattern (v1.5 prompt + Tier 2 Capa A+B+C) intact under D2; Auditor Layer (c) refinement does not change §6 enforcement boundary at Layer (a) validator or Layer (b) Finding-Lenient. redteam-smoke **0.92** carry preserved (= v0.1.14-v0.1.24 frozen baseline; aggregation policy refinement does not regress deterministic adversarial patterns by construction). **§6 interpretive evolution — THREE-layer Auditor architecture (centerpiece)**: v0.1.25 evolves §6 from "byte-equivalent semantics + additive observability" (v0.1.24 ADR-0031) to **"THREE-layer Auditor architecture: validator + Finding-Lenient BYTE-UNCHANGED + Turn-level aggregation policy MODIFIED at Layer (c) partial routing"**. Layer (a) per-citation validator (`citation/validator.py` + `citation/schemas.py`) BYTE-UNCHANGED at v0.1.25 (verified by `git diff main -- src/regulaitor/citation/` empty at T8); Layer (b) Finding-level Lenient aggregation (`auditor.py` `any(r.validated)` line ~61) BYTE-UNCHANGED; Layer (c) Turn-level aggregation policy gains one ADDITIVE conditional branch at the partial sub-route (pre-v0.1.25 RHR routing preserved on the non-paraphrase code path; NEW PASS code path gated on binary all-paraphrase-only condition derived from Layer (a) observability field). Fabrication-detection chain through Layer (a) + Layer (b) UNBROKEN by construction: any Check 1 or Check 2 invalid in any blocked Finding → helper returns False → RHR (preserves pre-v0.1.25 routing). v0.1.25 is the **SECOND interpretive evolution of the §6 statement** in the H4→v0.1.25 lineage (v0.1.24 was the first); the contract strengthens, not weakens, by the explicit interpretive precision. **7 §22.22 disclosures (verbatim summary)**: (1) context_precision -0.02 + faithfulness -0.01 + answer_relevancy -0.03 are population-composition mechanical artifacts (more PASS cases counted in mean), all still bar-PASS, NOT response-quality regressions; (2) chat-016 NEW BLOCK escalation (gold=pass) — all-blocked routing UNCHANGED at v0.1.25 D2; carry-forward to v0.1.26+ if needed (Design D territory); (3) cost_per_chat €0.054 over bar €0.05 by €0.004 (Capa C retry overhead carry); (4) 1-arm vs cached baseline methodology trade-off (~2-day API drift acknowledged; mitigated by same-cohort same-prompts same-retrieval same-models comparison; ~20% noise floor per v0.1.23 §REVERT root cause #1); (5) per-citation 5-bucket diagnostic counts STRICT-validated only; the 9/10 H1 flip rate via verdict-flip-review is the empirical bridge; (6) 0 RHR verdicts in cohort doesn't mean the capability is gone (auditor.py still routes RHR for non-Check-3 failures per D2 helper); the cohort happens not to exercise that path; (7) cumulative-impact measurement not per-capability attribution (D2 layered on v0.1.22-cumulative state; surgical D2 ablation would require additional FRESH arm not in v0.1.25 scope; carry-forward to H17 if needed for TFM defense). HARD invariants intact: §6 `citation/validator.py` + `citation/schemas.py` BYTE-UNCHANGED + auditor.py Finding-Lenient layer BYTE-UNCHANGED (only the partial-Findings aggregation branch modified) + Council + Analyst prompts v1.0-v1.5 + Pydantic schemas + eval pipeline + gold set + retrieval ALL BYTE-UNCHANGED. 1 src/ file modified (`agents/auditor.py` D2 helper + 1-branch wiring). ADR-0032 (count: 31 → 32). 7 new $0 unit tests in `tests/unit/agents/test_auditor.py` (4 helper-level + 3 integration). Gate autoritativo: `uv run pytest -m "not slow"` → **979 passed / 0 failed / 1 skipped** (= 972 baseline at v0.1.24 + 7 new at T1) + `uv run mypy src` Success 71 source files exit 0 UNCHANGED (no new `.py` under src/; helper added to existing `agents/auditor.py`) + redteam-smoke 0.92 carry + coverage ≥85% (~88.55% baseline; carry-forward to H16). **10 consecutive milestones with §22.22 honest framing pattern** (v0.1.19 / v0.1.20 / v0.1.21 / v0.1.21.2 / v0.1.22 / v0.1.22.1 / v0.1.23 / v0.1.24 / v0.1.24.1 / v0.1.25): v0.1.23 was the REVERT outcome; v0.1.25 is the CONFIRM outcome closing the SAME lineage (v0.1.22.1 H1 attribution → v0.1.24 O2 H1.C decomposition → v0.1.24.1 Path B layer attribution → v0.1.25 Design H D2 intervention at the diagnosed layer). The **diagnose-intervene-decompose-CONFIRM 4-round cycle is vindicated**: the same diagnostic discipline that produced v0.1.23 REVERT (right diagnosis, wrong layer) produced v0.1.25 CONFIRM (right diagnosis, right layer, right time). **TFM defense framing**: the v0.1.22.1 → v0.1.23 (REVERT) → v0.1.24 (alignment + instrumentation) → v0.1.24.1 (layer attribution) → v0.1.25 (CONFIRM) sequence is the **complete diagnose-intervene-measure-refute-revert-decompose-CONFIRM science cycle**, vindicating the methodology across both REVERT and CONFIRM outcome directions; the §6 invariant held throughout (now THREE-layer architecture-honest). **The methodology continues to be the contribution.** Sin skills nuevas. Ver `docs/technical_decisions_log.md` §v0.1.25 + ADR-0032 + `evals/reports/v0.1.25/` (6 reports: probe + v0.1.25-prod + v0.1.25-prod-main + comparison + per-citation-mechanism + verdict-flip-review) + spec + plan.

### Hito siguiente

- **v0.1.29** — Auditor all-blocked routing softening (Design D Mirror of v0.1.25 D2) cerrado (2026-05-27, squash `0378f4e`, tag `v0.1.29-chat-016-all-blocked-softening`, ADR-0034). Paid €1.89 (€0.14 sunk T5 probe 1 + €0.31 fixed T5 probe 2 + €1.44 T7 main; budget post ~$9.20 USD remaining). **Scope (1 src/ file)**: REUSE existing `_all_blocked_findings_paraphrase_only` helper (added v0.1.25 ADR-0032) at the all-blocked branch in `src/regulaitor/agents/auditor.py:elif all(v == "blocked")`; 1-branch wire-up (`PASS if helper else BLOCK`); helper docstring updated to mention both call sites (v0.1.25 partial + v0.1.29 all-blocked). +3 integration tests in `tests/unit/agents/test_auditor.py`. **HEADLINE: verdict_match +0.08** (0.68 → 0.76 on H10 main 25-case cohort chat-006..030; ON-FORECAST per ADR-0034 D4 prediction +0.033 to +0.10 = 1-3 wins; actual +2 net real wins). **chat-016 BLOCK → PASS as predicted ✅** (D Mirror canonical case). **2 bonus flips**: chat-020 (BLOCK→PASS; another Bucket B case) + chat-029 (PASS→BLOCK; correctly catches refusal). citation_recall ASPIRATIONAL ≥0.80 PRESERVED at 0.81 carry from v0.1.25. **7/7 v0.1.20-bar PASS** preserved. **§6 evolution — Layer (c) modified at BOTH sub-routes**: validator + Finding-Lenient BYTE-UNCHANGED; aggregation policy now modified at partial (v0.1.25 D2) AND all-blocked (v0.1.29 D Mirror) sub-routes via shared helper; both preserve fabrication blocking (any Check 1/2 → helper False → BLOCK by construction); only Check 3 paraphrase-only mismatches route PASS. §6 risk MEDIUM-HIGH (higher than v0.1.25 D2 because PASS routing now possible when EVERY citation strict-fails) — mitigated by binary helper condition + §6 verification via reason-text semantic + live-behavior inference. **Pre-existing LANCEDB_PATH config bug discovered during T5 probe 1** (€0.14 sunk): v0.1.26 deploy-prep added env-reading to `rag/store.py`; `.env` value `./corpus/indexes` pointed at parent dir → lancedb created empty `chunks.lance/` table → 0 rows retrieved → Sonnet refused → 'UNKNOWN' citations; fixed by setting `.env` to `./corpus/indexes/regulaitor.lance` + deleting stray empty table. Stage 1 cleanup commit bundles 6 docs-$0 items: CLAUDE.md §6.1 multi-layer summary + auditor.py inline ADR refs + council.py docstring update + pre_h16_review.md §11 status update + LANCEDB_PATH post-mortem + `evals/metrics.py` per_citation_audits trail-fix (5 LOC; restores `failed_check` field that was dropped pre-v0.1.24 O2). **§22.22 disclosures (7)**: marginal lift confirmed on-forecast + LANCEDB_PATH config bug found pre-paid + trail-fix bundled + 2-day API drift acknowledged + 1-arm vs cached methodology + cost overhead Capa C carry + cumulative state (not isolated D Mirror). HARD invariants intact: src/ scope = exactly 1 file (`agents/auditor.py`); §6 `citation/` byte-unchanged; Layer (b) byte-unchanged; eval pipeline byte-unchanged except metrics.py trail-fix aditivo; gold set byte-unchanged. Gate autoritativo: `uv run pytest -m "not slow"` → **999 passed / 0 failed / 1 skipped** (= 996 v0.1.28 baseline + 3 new D Mirror tests) + `uv run mypy src` Success 71 source files exit 0 UNCHANGED + redteam-smoke 0.92 carry + coverage 88.62% ≥ 85%. **11 consecutive §22.22 milestones** (v0.1.19 / v0.1.20 / v0.1.21 / v0.1.21.2 / v0.1.22 / v0.1.22.1 / v0.1.23 / v0.1.24 / v0.1.24.1 / v0.1.25 / v0.1.29). The v0.1.25 (partial) + v0.1.29 (all-blocked) pair exhausts the LOW-MEDIUM §6 risk surface at the Auditor aggregation layer Layer (c). **Closes the v0.1.25 CLAUDE.md §27 CONDITIONAL carry-forward** for chat-016-like all-blocked paraphrase-only cases. Carry-forwards: (a) doc backfill v0.1.26/v0.1.27/v0.1.28 in decisions_log + evidence_matrix + CLAUDE.md §27 — deferred to next session OR H17 cierre; (b) v0.1.30 title-augmented embeddings (NEXT, user-approved per Stage 2 ordering, ~€2 expected); (c) pre-H16 polish $0 Stage 3; (d) H16 deploy Stage 4; (e) H17 TFM cierre Stage 5. **TFM defense framing**: v0.1.29 D Mirror is the symmetric closure of v0.1.25 D2's lineage. Both ship the same helper at different sub-routes; both preserve §6 invariant via the same binary all-Check-3 condition; both deliver predicted verdict_match lifts (D2: +0.33 at partial; D Mirror: +0.08 at all-blocked). The asymmetric magnitudes reflect cohort-specific frequencies (D2 affected ~9/30; D Mirror affects ~3-5/30). The methodology continues to be the contribution. Sin skills nuevas. Ver ADR-0034 + `evals/reports/v0.1.29/` (4 reports: probe + v0.1.29-prod-main + comparison + verdict-flip-review) + `docs/technical_decisions_log.md` §v0.1.29.

- **v0.1.30** — Title-augmented corpus embeddings (ACCEPTED then REVERTED per T6 probe empirical refutation) cerrado (2026-05-28, squash `<squash-sha>`, tag `v0.1.30-title-augmented-embeddings`, ADR-0035 retained with §REVERT). Paid €0.65 total (T5 probe sunk; T7 main SKIPPED per cost-discipline ~€1.40 saved). **Scope (1 src/ file at activation)**: NEW `_text_to_embed(chunk, parsed)` helper + 1-line modification in `src/regulaitor/rag/build.py` (`embed([_text_to_embed(ch, parsed) for ch in chunks])` instead of `embed([ch.text for ch in chunks])`). Mirror at corpus-side of v0.1.28 T4-bis title-prepend query-side; bridges descriptive-doc-segment → obligation-corpus-article semantic gap by encoding article topic identity in corpus vectors. T0 controller branch + snapshot pre-v0.1.30 index aside + T1+T2 helper + 1-line + 5 unit tests (commit `0afe9d1`) + T3 rebuild full corpus $0 (1569 chunks × BGE-M3 local; ~1.5h CPU wall-clock; HF_HUB_OFFLINE=1 bypass for Windows CryptoAPI CRL SSL bug per ADR-0029 §22.22 #2) + T4 ADR-0035 (~165 lines; D1-D5 + 4 alternatives + 5 §22.22 disclosures + §6 LOW risk; commit `e91a690`) + T5 paid probe (€0.65 actual / €0.40 expected; doc N=3 + chat N=3) + T6 REVERT decision + T-revert (atomic snapshot mv-back + manifests git-checkout + code restoration + 5 tests removed; commit `48c4923`). **§22.22 HEADLINE — probe empirical refutation**: doc-mode citation_recall 0.33 flat (target ≥0.38 per ADR-0035 D5 SHIP criterion was +0.05 lift); **doc-001 REGRESSED precision 0.50 → 0.00**; over-citation expansion median 5x (doc-001 1-2 → 12; doc-003 1 → 19); chat-mode chat-001..003 essentially unchanged (3/3 verdict preserved; chat-002 slightly improved). **Mechanism (§22.22 honest)**: title-augmented embeddings surface significantly more topic-related corpus articles per doc segment → v1.6 doc_analyst emits Findings citing all surfaced articles → precision tanks because gold-specific articles still don't dominate the surfaced set + over-emission dilutes the signal. **This is the same mechanism as v0.1.28 T4-extra α+β REVERT** (ADR-0033 §22.22 #5; top_k=15 + max_chunks=5 caused context dilution → citation_precision 0.17 → 0.00). The breadth dilution at the retrieval-config layer (v0.1.28 T4-extra) and at the embedding-vector layer (v0.1.30) produce the same over-citation failure mode. Structural to BGE-M3 + v1.6 doc_analyst combination, not stochastic to the specific intervention. **T7 main SKIPPED**: structurally clear probe + matches prior REVERT mechanism + cost discipline (~12% of remaining budget saved; same outcome expected with ~85% confidence). **§6 invariant HELD throughout**: `citation/validator.py` + `citation/schemas.py` + `agents/auditor.py` + `agents/analyst.py` + `prompts/` + `rag/chunking.py` + `rag/retrieval.py` + `rag/schemas.py` + `rag/store.py` + `evals/` pipeline + gold set ALL BYTE-UNCHANGED across both T1+T2 (activation) and T-revert (restoration). 0 fabrications detected at probe (per-citation reasons all valid `text_not_in_apartado` or `article_not_found` patterns). redteam-smoke 0.92 carry preserved by construction. Live index post-revert verified cosine sim 0.97 (NOT 1.0) vs discarded v0.1.30 index on ai_act.1.en sample → atomic restoration is real. **Lessons learned (5 carry-forwards)**: (a) over-citation pattern structural to BGE-M3 + v1.6 doc_analyst combo when retrieval breadth expands at ANY layer (top_k, max_chunks_per_norma, embedding broadening); (b) descriptive-doc-segment → obligation-corpus-article semantic gap fundamental at embedding level — CANNOT be closed by title prefix alone; HyDE (Alternative A in ADR-0035) HX post-deploy work; (c) v0.1.28 T4-bis query-side title-prepend STAYS (proven helpful at v0.1.28 main; citation_recall 0 → 0.33; the v0.1.30 REVERT does NOT revert v0.1.28's query-side prepend); (d) doc gold set N=10 too small for high-confidence retrieval-engineering decisions — HX retrieval work needs N≥30 doc cases OR real production traffic, neither available pre-H17; (e) methodology vindication — v0.1.30 is the **2nd REVERT outcome** in the §22.22 lineage (after v0.1.23). HARD invariants intact post-REVERT: src/ scope = 0 files modified vs main (build.py restored byte-equivalent); 5 tests removed (added then deleted in squash); 1 ADR added (ADR-0035 with §REVERT section retained). Gate autoritativo: `uv run pytest -m "not slow"` → **999 passed / 0 failed / 1 skipped** (back to v0.1.29 baseline; -5 tests removed at T-revert; net delta vs main = 0 src/ changes) + `uv run mypy src` Success 71 source files exit 0 UNCHANGED + redteam-smoke 0.92 carry + coverage 88.62% ≥ 85%. **12 consecutive §22.22 milestones with 2 REVERTs in lineage** (v0.1.19 / v0.1.20 / v0.1.21 / v0.1.21.2 / v0.1.22 / v0.1.22.1 / v0.1.23 [REVERT Auditor lenient quorum] / v0.1.24 / v0.1.24.1 / v0.1.25 [CONFIRM partial-routing] / v0.1.29 [CONFIRM all-blocked] / v0.1.30 [REVERT title-augmented]); 2 REVERT outcomes both shipped honestly with §REVERT sections + atomic restoration. **TFM defense framing**: v0.1.30 REVERT is the 2nd empirical refutation in the §22.22 lineage. The diagnose → intervene → measure → refute → revert → document science cycle applies across BOTH Auditor-layer (v0.1.23) and retrieval-layer (v0.1.30) interventions. **The asymmetry (query-side prepend helps; corpus-side prepend hurts) is the substantive scientific finding of v0.1.30** — worth documenting in H17 memoria as a non-obvious empirical insight about retrieval-vs-emission dynamics in v1.6 doc_analyst. The methodology continues to be the contribution. Sin skills nuevas. Ver ADR-0035 (with §REVERT section) + `evals/reports/v0.1.30/probe.md` (€0.65 sunk evidence) + `docs/technical_decisions_log.md` §v0.1.30.

- **H16 — Despliegue público (HF Spaces + foundation production-grade per "future product" preference)** — RECOMMENDED DEFAULT post-v0.1.30 REVERT (v0.1.30 closes the retrieval-engineering-attempt window pre-deploy; remaining doc-mode lift candidates HyDE / hybrid BM25 / custom reranker are HX post-deploy work informed by real traffic). v0.1.25 closed as **CONFIRM outcome** (paid €1.66 / ~$1.80 USD; verdict_match +0.33 lift — the LARGEST verdict_match lift in the H1→v0.1.24 lineage; 7/7 v0.1.20-bar PASS; 9/10 H1 cases flipped RHR → PASS as predicted by v0.1.24.1 Path B 8/10 — empirical antithesis of v0.1.23 Design B 0/10; Bucket C=0 zero Tier 1 quorum RHR on the cohort; safety floor PASS chat-014/015/029/030; §6 THREE-layer architecture interpretive evolution — validator + Finding-Lenient BYTE-UNCHANGED + aggregation policy modified at Layer (c) only; SECOND interpretive evolution of §6 following v0.1.24's first; 10 consecutive §22.22 milestones; diagnose-intervene-decompose-CONFIRM 4-round cycle closes the v0.1.22.1 → v0.1.24 → v0.1.24.1 lineage that v0.1.23 REVERT had opened). v0.1.25 + v0.1.24 + v0.1.23 carry-forwards documented: (a) chat-016 all-blocked routing investigation (LOWER priority post-CONFIRM — Design D territory; only pursued at v0.1.26 if all-blocked softening becomes critical post-deploy); (b) Designs A/C HX-deferred (validator-direct OR schema field if verdict_match becomes critical post-deploy); the LOW-MEDIUM §6 risk surface at the Auditor aggregation layer is exhausted in v0.1.25 D2; (c) Gold expected_verdict update for H15 C1 pattern (CLOSED at v0.1.24 O1 via `acceptable_verdicts` per-case opt-in for 6 designated content safety cases); (d) AuditResult schema extension `failed_check: Literal[1,2,3,None]` (CLOSED at v0.1.24 O2; READ by v0.1.25 D2 helper); (e) Coverage gate threshold adjustment (88.55% → 85% OR fix offline-SSL test path); (f) truststore in `pyproject.toml` (currently `.venv` only); (g) per-capability cost attribution still unmeasured (carry from ADR-0029; v0.1.25 measures cumulative not parts). v0.1.22 paid validation CLOSED (CONDITIONAL CONFIRM per ADR-0029; cumulative v0.1.19→v0.1.21.2 capability arc empirically validated; NEW Tier 1 quorum mechanism fires 36.7% of cohort; Capa A+B+C 100% effective; hard safety floor PASS). v0.1.22.1 verdict-drop diagnostic CLOSED (H1 dominant 62.5%; refuted at v0.1.23 at the intervention-layer level; confirmed at v0.1.24 at the sub-bucket-attribution level; resolved at v0.1.25 at the right layer = Layer (c) partial routing per v0.1.24.1 Path B attribution + 9/10 flip rate verification). Dual deploy: HF Spaces (demo TFM, gratis) + Render/Fly.io setup (foundation reusable). Docker compose serio + secrets manager + health checks + CORS/CSP. ~1 semana.

- **v0.1.26 (CONDITIONAL — only if needed post-deploy)** — All-blocked-Findings routing softening (Design D territory; targets chat-016-like cases where all 3 emitted citations fail strict validator → all-blocked branch routes to BLOCK). LOWER priority than H16 deploy because: (a) v0.1.25 D2 exhausted the LOW-MEDIUM §6 risk surface at the partial-Findings sub-route; v0.1.26 would touch the all-blocked sub-route which has no Finding-pass evidence to anchor on — HIGHER §6 risk than D2 (closer to Designs A/C HX territory); (b) only 1/30 case (chat-016) was affected on the H10 cohort; (c) the all-blocked → BLOCK route is the §6-safe destination by construction when no Finding has any STRICT-valid citation. Pursued ONLY if real-world need surfaces post-deploy.

- **H17 — Cierre académico**: memoria, model card, data card, AI Act assessment, runbook, cost analysis, video demo, slide deck, evidence matrix completa + apéndice "Product Roadmap" (per "future product" preference) + **§"v0.1.23 REVERT — methodology as contribution"** subsection (the 7-consecutive-§22.22 lineage + the diagnose-intervene-measure-refute-revert-document science cycle as the TFM's narrative spine). Tag `v1.0.0`. ~1-2 semanas post-H16.

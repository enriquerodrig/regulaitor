...
- **v0.1.24.1** — Per-Finding Auditor path diagnostic ($0 mini-milestone) cerrado (2026-05-26, squash `<squash-sha>`, tag `v0.1.24.1-finding-path-diagnostic`). $0 cross-version comparison diagnostic mirrors v0.1.21.1 / v0.1.21.3 light pattern (NO new ADR; NO src/ touch). Categorizes 10 v0.1.22.1 H1 cases by Auditor PATH via v0.1.22-prod vs v0.1.23-prod actual_verdict comparison. **HEADLINE: Path B (Strict-Answer partial routing) DOMINANT at 8/10 = 80%**; Path A=0/10 (Tier 1 confirmed not the layer per v0.1.23 REVERT); Path C-ish=2/10 (chat-016, chat-017 RHR→BLOCK; ambiguous all-blocked vs API drift). NEW `scripts/v0124_1_finding_path_diagnostic.py` (257 lines; ruff + black + mypy clean) + report `evals/reports/v0.1.24.1/finding-path-attribution.md`. **v0.1.25 Design H locked-in recommendation**: Strict-Answer partial-Findings routing softening (line ~70 auditor.py). MEDIUM §6 risk (changes routing not validation; Finding-Lenient + Tier 1 + validator stay byte-equivalent). NEW ADR-0032. Paid mini-validation ~€2-3 to confirm verdict_match lift. **§22.22 caveats** (5 in report): cross-version inference ~20% API drift noise floor; Path C-ish ambiguity; per-Finding citation grouping lost in cached AuditResults; design selection still requires user judgment. **9 consecutive milestones with §22.22 honest framing** (v0.1.19 / v0.1.20 / v0.1.21 / v0.1.21.2 / v0.1.22 / v0.1.22.1 / v0.1.23 / v0.1.24 / v0.1.24.1). HARD invariants intact: NO src/ touch + NO test additions + NO gold update + NO evals/ pipeline change + NO new ADR. Gate UNCHANGED: 972/0/1 + mypy 71 Success + redteam-smoke 0.92 + coverage 88.55%. Ver `evals/reports/v0.1.24.1/finding-path-attribution.md`.

### Hito siguiente

- **v0.1.25 (CONDITIONAL) — Design H: Strict-Answer partial-Findings routing softening (MEDIUM §6 risk; ADR-0032; ~3-5 días $0 implementation + ~€2-3 paid mini-validation)** — LOCKED IN per v0.1.24.1 finding-path diagnostic (Path B DOMINANT 8/10 = 80% confirms Strict-Answer routing is the firing layer for H1.C cases). Currently `auditor.py` partial-Findings routing → RHR; v0.1.25 change: partial-Findings + ≥X% Findings pass + most invalids are failed_check==3 → PASS (loosens routing not validation). Targets the empirical gatekeeper without touching Finding-Lenient or Tier 1 layers (LOWER §6 surface than Designs G or D). Validation: paid mini-validation A/B on H10 30-case (~€2-3) measures verdict_match lift. CONFIRM if ≥+0.10 + safety floor PASS; CONDITIONAL CONFIRM if mixed; REVERT if §6 regression (precedent v0.1.23 §REVERT). Decision-tree dual outcome documented in ADR-0032.

- **H16 — Despliegue público (HF Spaces + foundation production-grade per "future product" preference)** — default if v0.1.25 deferred. v0.1.24 closed as **alignment + instrumentation milestone** ($0 throughout; O1 acceptable_verdicts +0.10 verdict_match lift on H10 30-case via re-aggregation; O2 AuditResult failed_check field + decomposition diagnostic confirms 10/10 H1 cases are H1.C Check 3 paraphrase-only mismatch; v0.1.22.1 H1 attribution was CORRECT; v0.1.23 REVERT was right call for wrong reason; §6 invariant evolves from byte-unchanged to byte-equivalent semantics + additive observability — FIRST interpretive evolution in H4→v0.1.24 lineage). v0.1.23 closed as **REVERT outcome** (Design B Auditor lenient quorum SHIPPED + paid-validated + empirically REFUTED at T6 + REVERTED at T-revert per spec §D5 third path; verdict_match REGRESSED -0.03 vs predicted +0.10; 0/10 H1 cases flipped as predicted; §6 invariant HELD throughout); production state IDENTICAL to v0.1.22.1 baseline. v0.1.24 + v0.1.23 carry-forwards documented: (a) v0.1.25+ Finding-Lenient OR Strict-Answer routing intervention (CONDITIONAL above; may be pursued post-H16 if deferral preferred); (b) Designs A/C HX-deferred (validator-direct OR schema field if verdict_match becomes critical post-deploy); (c) Gold expected_verdict update for H15 C1 pattern (CLOSED at v0.1.24 O1 via `acceptable_verdicts` per-case opt-in for 6 designated content safety cases); (d) AuditResult schema extension `failed_check: Literal[1,2,3,None]` (CLOSED at v0.1.24 O2); (e) Coverage gate threshold adjustment (88.55% → 85% OR fix offline-SSL test path); (f) truststore in `pyproject.toml` (currently `.venv` only). v0.1.22 paid validation CLOSED (CONDITIONAL CONFIRM per ADR-0029; cumulative v0.1.19→v0.1.21.2 capability arc empirically validated; NEW Tier 1 quorum mechanism fires 36.7% of cohort; Capa A+B+C 100% effective; hard safety floor PASS). v0.1.22.1 verdict-drop diagnostic CLOSED (H1 dominant 62.5%; refuted at v0.1.23 at the intervention-layer level; confirmed at v0.1.24 at the sub-bucket-attribution level). Dual deploy: HF Spaces (demo TFM, gratis) + Render/Fly.io setup (foundation reusable). Docker compose serio + secrets manager + health checks + CORS/CSP. ~1 semana.

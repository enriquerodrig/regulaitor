...
- **H9** — Red team inicial cerrado (YYYY-MM-DD). Tag `v0.0.10-h9` publicado. Squash commit `<sha>` en main. ADR 0011. 50 ataques sobre los 10 escenarios §18 (22 chat-mode + 28 doc-mode; ~15 con requires_e2e). Runner standalone Python (`redteam/runner.py`) + `make redteam-smoke` CI job ($0, ~30s) + full run ~$3.31. block_rate_final: `<X.XX>` (gate §16.2 #4 ≥ 0.90). Skills `redteam-runner` v1.0 + `secure-coding-checklist` v1.0 activadas. Ver `docs/technical_decisions_log.md` §H9.

### Hito siguiente

- **H10** — Documentación MVP: README completo, `docs/architecture.md` con C4 Mermaid, ADRs al día, `docs/evidence_matrix.md` poblada, `make setup && make ingest && make eval && make redteam && make serve` reproducibles desde clone limpio. Sin nuevo código de producción — entregables docs + congelación MVP + tag `v0.1.0-mvp`.

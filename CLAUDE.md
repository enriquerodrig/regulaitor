...
- **`v0.1.15` — Gap-analysis chat mode** (user-approved insertion 2026-05-21, post-v0.1.14 measurement de TFM-dual-target requirement: production-ready precision para LinkedIn publish + AI industry presencial session). Extiende sistema a tres surfaces: chat Q&A (existente) + **chat gap-analysis** (NEW v0.1.15: user NL input "X aplica, tengo Y, ¿qué falta?" → structured gap list grounded in citations) + document audit (existente H5 doc-mode con PDF input). Architecture: Analyst prompt v1.2 → v1.3 con few-shot gap-analysis examples + ~5-10 gap-analysis cases al gold set (industry-gN naming similar) + opcional new API endpoint `POST /gap-analysis` con structured input. §6 invariant compatible: cada claim "qué exige la ley" cited from corpus; user's declared state es INPUT (no normative claim); gap IS the difference (reasoning). Medium ceremony (~1-2 días $0). Backward-compat: Analyst v1.2 stays available via env override `REGULAITOR_ANALYST_PROMPT_VERSION`; production default decides at v0.1.20 paid validation. Tras `v0.1.15`: secuencia `v0.1.16` (§17 thresholds + LLM-judge same-provider) · `v0.1.17` (no-Answer-residual robustness) · `v0.1.18` (citation granularity confound) · `v0.1.19` (Auditor RHR + Council binding) · `v0.1.20` (single paid validation A/B cuando recargue budget) · luego retorno a **H16** (HF Spaces deploy para live URL) + **H17** (TFM cierre académico memoria + slides + video + anexos). Sistema ya defendible HOY con limitaciones honestas; los microhitos restantes amplían valor industry-demo sin bloquear deployability.

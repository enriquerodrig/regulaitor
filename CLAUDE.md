...
- **H15.1** — Optimización system-level (insertado post-H15, decimal sin renumerar — precedente H0.1; decisión de roadmap aprobada por el user 2026-05-19, ver `docs/technical_decisions_log.md` §H15.1). Objetivo: subir la **calidad real del sistema** atacando el techo system-level que el estudio H15 documentó (no metric-gaming, §22.22 — "métricas" = que el sistema funcione lo más preciso posible). Palancas nombradas en ADR-0016 / decisions §H15: re-tuning del **retriever (lever C)** (context_precision ~0.60 < 0.80, palanca dominante remanente; medición-solo en H15 por D2/D5), **segmentador documental** (el probe 1-doc emitió 0 segmentos → doc-mode A/B incomputable, 10 doc-holdout diferidos), **no-Answer residual** (2/14 holdout empty-answer; intervención B redujo pero no eliminó), y la **semántica RHR-aggregation del Auditor** (sobre-dispara RHR en ambiguos; H13 57% divergencia Council; el seam `MonotonicEscalatePolicy`/`_COUNCIL_BINDING` sigue OFF). Backend H1-H3 read-only salvo lo que el diseño (brainstorming→spec→writing-plans→subagent-driven-development) justifique y registre en un ADR nuevo. Disciplina A/B baseline-congelada (la baseline v0.1.5-h15 es el control) + §22.22. Después: **H16** Despliegue público (HF Spaces) — más defendible con retriever/segmentador arreglados — y **H17** cierre académico (ambos sin renumerar). Tag previsto `v0.1.6-h15.1`. El sistema ya es publicable con limitaciones honestas (invariante §6 100%, seguridad DURA verificada 6/6 + redteam-smoke 0.92); H15.1 sube calidad antes de exponerlo.

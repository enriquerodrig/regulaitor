...
- **H15.X** (microhitos optimización, plan maximalista user-confirmed 2026-05-20, todos $0 implementación): `v0.1.8` harness checkpoint per-case (MANDATORY first) · `v0.1.9` xcorpus-002 + retriever local re-tuning · `v0.1.10` gold-set extension auto-path · `v0.1.11` §17 thresholds + LLM-judge same-provider · `v0.1.12` no-Answer-residual · `v0.1.13` document segmenter overhaul · `v0.1.14` citation granularity confound · `v0.1.15` Auditor RHR + Council binding · `v0.1.16` single paid validation A/B (when budget recharges).

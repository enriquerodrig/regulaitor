"""v0.1.25 — paid validation runner (mirrors scripts/v0120_run.py).

Routes harness report output to evals/reports/v0.1.25/<tag>.md after each run,
...
v0.1.25 difference vs v0120_run.py: NO env override — production state means
...
the spec's SKIP/PROCEED gate (T2) authorizes. Invoke as:
  ...
  uv run --env-file .env python -m scripts.v0125_run \\
      --cases-file evals/v0125_probe_ids.txt --tag probe

  uv run --env-file .env python -m scripts.v0125_run \\
      --cases-file evals/v0125_main_ids.txt --tag v0.1.25-prod-main

  uv run --env-file .env python -m scripts.v0125_run \\
      --cases-file evals/v0125_safety_adhoc_ids.txt --tag v0.1.25-prod-safety-adhoc
"""

from __future__ import annotations

# v0.1.25 infra fix: Windows CryptoAPI CRL revocation check fails on this machine
...
_V0125_DIR = Path("evals/reports/v0.1.25")
...
def _isolate_report(tag: str) -> None:
    """Path-B isolation: snapshot this run's report to evals/reports/v0.1.25/<tag>.md,
    ...
    """
    _V0125_DIR.mkdir(parents=True, exist_ok=True)
    dest = _V0125_DIR / f"{tag}.md"
...
def run(*, cases_file: Path, tag: str) -> None:
    """Run the harness for v0.1.25 paid validation with PRODUCTION STATE env.
    ...
    tag: report basename under evals/reports/v0.1.25/ (e.g. "probe",
         "v0.1.25-prod-main", "v0.1.25-prod-safety-adhoc").
    """
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="v0.1.25 USER-GATED paid validation runner")
    p.add_argument(
        "--cases-file",
        type=Path,
        required=True,
        help="Newline-delimited case-id allowlist (e.g. evals/v0125_probe_ids.txt)",
    )
    p.add_argument(
        "--tag",
        required=True,
        help="Report basename under evals/reports/v0.1.25/ (e.g. probe, v0.1.25-prod-main)",
    )
    return p.parse_args()

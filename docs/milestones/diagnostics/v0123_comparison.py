...
"""T2 Deliverable: Compare v0.1.23-prod vs v0.1.22-prod baseline ($0 cache analysis).

Reads:
  - evals/reports/v0.1.23/v0.1.23-prod.md (30 cases merged)
  - evals/reports/v0.1.22/probe.md + v0.1.22/v0.1.22-prod-main.md (cached baseline)

Outputs:
  - evals/reports/v0.1.23/comparison.md
...
def main():
    """Compare v0.1.23-prod vs v0.1.22-prod."""
    # Load v0.1.23-prod
    v0123_path = Path("evals/reports/v0.1.23/v0.1.23-prod.md")
    if not v0123_path.exists():
        raise FileNotFoundError(f"v0.1.23-prod not found: {v0123_path}")

    v0123_cases = extract_report_cases(str(v0123_path))
    ...
    # Compute metrics
    v0123_metrics = compute_metrics(v0123_cases)
    ...
    # Verdict counts
    v0123_verdicts = {
        "pass": sum(1 for c in v0123_cases.values() if c["actual_verdict"] == "pass"),
        "requires_human_review": sum(
            1 for c in v0123_cases.values() if c["actual_verdict"] == "requires_human_review"
        ),
        "block": sum(1 for c in v0123_cases.values() if c["actual_verdict"] == "block"),
    }
    ...
    for metric_name in [
        ...
        report_lines.append(
            f"| {metric_name} | {v0123_val:.2f} | {v0122_val:.2f} | {delta_str} | "
    ...
    for metric_name in [
        ...
        pass_status = "PASS" if v0123_val >= bar_val else "FAIL"
        trend = "improvement" if delta > 0.005 else ("regression" if delta < -0.005 else "flat")

        report_lines.append(
            f"- **{metric_name}:** {v0123_val:.2f} vs baseline {v0122_val:.2f} "
    ...
    report_lines.extend(
        [
            ...
            "",
            f"v0.1.23-prod: pass={v0123_verdicts['pass']} "
            f"RHR={v0123_verdicts['requires_human_review']} "
            f"block={v0123_verdicts['block']}",
            "",
            ...
            "## Summary",
            "",
            f"**{sum(1 for m in [v0123_metrics.get(k, 0.0) >= thresholds.get(k, (0.0, 0.0))[0] for k in ['faithfulness_mean', 'answer_relevancy_mean', 'context_precision_mean', 'citation_precision_mean', 'citation_recall_mean', 'verdict_match_rate', 'severity_match_rate']])} / 7 metrics PASS bar in v0.1.23-prod**",
            "",
            "## Note on per-citation audits",
            "",
            "v0.1.23-prod has per-citation audit trail available per v0.1.21.1 D2 "
    ...
    # Write output
    output_path = Path("evals/reports/v0.1.23/comparison.md")

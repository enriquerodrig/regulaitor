...
### Capa §18 — injection detection mutation audit

Extensión del patrón al pilar de seguridad (`scripts/sec18_injection_mutation_audit.py` + CI-guard `test_sec18_injection_mutation_audit.py`): exec-once de una copia mutada de `security/injection.py` + batería de invariantes §18 (payloads de ataque conocidos DEBEN detectarse; queries benignas NO). **`injection.py` NO se modifica.** El redteam smoke (block_rate ≥0.90) cubre esto behaviorally; el audit fija la LÓGICA de detección a nivel de patrón. Review §18 (2 lentes): faithfulness lens SOUND (exec fiel, expected values correctos, baseline significativo); missed-weakening lens encontró **1 gap HIGH real** — la batería inicial (7 payloads) sólo cubría 7 de 25 patrones; **18 patrones sin payload** → una mutación que neutraliza cualquiera (ej. role-override-es "ahora eres", primera-clase ES §23; data-exfiltration; citation-poisoning) SOBREVIVÍA. Cerrado con **cobertura per-patrón completa (25/25)**: 1 payload por patrón nombrado, cada uno pinneado al nombre del patrón que debe cazarlo → neutralizar CUALQUIER patrón voltea la tupla `(detected, name)` → killed; + una **coverage assertion** (`uncovered_patterns == []`) para que un patrón nuevo no pueda shipear sin test; + 3 mutaciones nuevas explícitas (role-override-es typo, fake-delimiter neuter, data-exfiltration neuter) → **9 mutaciones, todas killed** + 1 low (exec fuera del try/except → un mutante que no compila daba traceback opaco en vez de fila ERROR; `_load` ahora lo captura). §6 invariant intacto (esto es §18, path distinto). Docstring corregido §22.22 (el claim de cobertura ahora se sostiene porque la cobertura es completa).

### Capa §18.8 — document sanitizer mutation audit

Cuarto audit (`scripts/sec18_sanitizer_mutation_audit.py` + CI-guard): exec-once de una copia mutada de `document/sanitizer.py` + batería de invariantes §18.8 (doc malicioso DEBE `DocumentBlockedError` con el reason correcto; doc limpio → `SanitizedDocument`; unicode tricks stripped). Cubre los 6 critical-blocks (JS/attachment/form/URI/metadata-injection/metadata-URL) + length floor + unicode strip. **`sanitizer.py` NO se modifica.** Review §18.8 (2 lentes): faithfulness SOUND (nice hallazgo: `SanitizedDocument.clean_text` tiene `min_length=50` = defense-in-depth; el `below_content_floor` case usa 43 chars de contenido que dan 55-char clean_text para superar el schema y aislar la contribución ÚNICA del floor → kill limpio, no crash); missed-weakening encontró **2 gaps reales**: (HIGH) el strip unicode se validaba con UN solo char (ZWSP) → **droppear el RLO bidi-override (U+202E, Trojan-Source) SOBREVIVÍA** filtrando el char a `clean_text`→LLM; (MEDIUM) la batería nunca inspeccionaba `sanitizer_log` → droppear el log de annotation-strip / hidden-text-strip era invisible. Cerrado con **cobertura per-member (6/6)** de `_UNICODE_TRICKS` (generada del set real; coverage assertion `uncovered_tricks == []`) + 3 casos de inspección de `sanitizer_log` (annotation/hidden/unicode-severity con `(category, severity)` esperados) + 3 mutaciones nuevas (drop-RLO, annotation-log-removed, hidden-log-removed) → **11 mutaciones, todas killed**.

...
`uv run pytest -m "not slow"` verde + mypy 116 files + ruff/black + redteam-smoke 0.92 carry. `docs/sec6_mutation_audit.md` = 11 killed / 1 equivalent / 0 unexpected · `docs/sec6_auditor_mutation_audit.md` = 8 killed / 0 unexpected · `docs/sec18_injection_mutation_audit.md` = 25/25 patrones + 9 killed / 0 unexpected.

Sin skills nuevas (`citation-validator` activa desde H4; `redteam-runner` desde H9). Ver `scripts/sec6_mutation_audit.py` + `scripts/sec6_auditor_mutation_audit.py` + `scripts/sec18_injection_mutation_audit.py` + los 3 CI-guards + `test_validator.py` (empty-guard ==4 + combining-mark) + CLAUDE.md §6.1 (sec6-01b + audits §6).
